...
`/new` を送信して新しいセッションを開始し、Sleep をトリガー。`/sleep` を送信すると新しいセッションを開始せずにメモリ統合を手動トリガーできます。
...
!!! tip "推奨：Telegram Bot"
    Riverse は画像、音声、ファイルを処理するため、Telegram Bot をメインインターフェースとして使用することを推奨します。`settings.yaml` にあなた固有の Telegram User ID を設定してください。
