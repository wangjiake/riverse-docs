...
> **トークンは一度だけ生成されます。** `./config/settings.yaml` に保存されるため、`config/` ディレクトリが存在する限り再確認は不要です。

## System ページの設定

ログイン後、**System** ページ（http://localhost:1234）を開きます。すべての設定は Web UI から行えます。設定ファイルの編集は不要です。

| 設定セクション | 設定できる内容 |
|--------------|--------------|
| **LLM** | AI プロバイダー（OpenAI / DeepSeek / Groq / Ollama）、モデル名、API キー、API エンドポイント |
...
!!! warning "サーバーにデプロイする前に必ずお読みください"
    ...
    Web ダッシュボード（ポート 1234）はアクセストークンで保護されており、ファミリーマルチユーザーモード（メンバー単位のデバイストークン、[ファミリーマルチユーザーモード](family-multi-user.md) 参照）に対応しています。ただし、**REST API（ポート 8400）と RiverHistory（ポート 2345）にはアカウント単位の認証がありません** — ローカル／信頼できるネットワークからのアクセスを前提としています。リモートサーバーにデプロイする場合は以下のルールに従ってください：
...
OpenAI 互換 API すべてに対応。起動後に **System** ページ（http://localhost:1234）で設定するか、初回起動前に環境変数で指定できます：
...
`jkriver/` ディレクトリの中に `data/` フォルダを作成し、エクスポートファイルを配置：

```
jkriver/
├── docker-compose.yaml
├── config/                    ← 初回起動時に自動作成（設定を保存）
└── data/                      ← エクスポートファイル用に作成
...
起動後、ほとんどの設定は `http://localhost:1234` の **System** ページで変更できます。以下の環境変数は初回起動時（`./config/settings.yaml` が作成される前）にのみ適用されます。
