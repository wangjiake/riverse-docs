...
会話が流入 ──→ 浸食 ──→ 堆積 ──→ 認知を形成 ──→ 流れ続ける
                │         │         │
                │         │         └─ 確認された認知 → 安定した岩盤
                │         └─ 重要な情報 → 観察 + プロフィール事実（suspected → confirmed）
...
重要情報は沈泥のように堆積：感情や生信号は観察に、アイデンティティやパターン性のある判断は「プロフィール事実」となり、初出時は suspected（疑い）、繰り返し言及されると confirmed（確認済み）に昇格する。繰り返し確認された認知はより深く沈む。
